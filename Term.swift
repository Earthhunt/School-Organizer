...
class Term {
    ...
    init(name: String,
         gradeSystem: GradeSystem = .grades16,
         createdAt: Date = .now) {
        self.name = name
...
extension Term {
    ...
    var subjects: [(name: String, colorHex: String)] {
        var seen = Set<String>()
        var result: [(String, String)] = []
        for lesson in sortedLessons where !lesson.subject.isEmpty {
            if !seen.contains(lesson.subject) {
                seen.insert(lesson.subject)
                result.append((lesson.subject, lesson.colorHex))
            }
        }
        return result.map { (name: $0.0, colorHex: $0.1) }
    }
}
